...
- `main.py`: GUI, 이미지 변환 로직, 언어 재시작 로직
...
- 지원값: `en`, `ko`
- 첫 실행 기본값: `en`
- 언어 변경 시 GUI 프로세스를 재시작

재시작하는 이유:

- 전체 창을 새 언어로 확실하게 다시 그릴 수 있습니다.
- 기존 위젯에 남아 있는 오래된 문자열을 피할 수 있습니다.
- 패키징된 앱에서도 동작이 단순하고 예측 가능합니다.
...
- 언어 변경은 위젯을 부분 갱신하는 대신, 프로세스를 다시 띄워서 처리합니다.
